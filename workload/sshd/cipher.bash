...
echo
echo "For each cipher, will transfer 800 MB of urandom to/from ${remote}."
echo
...
do
  echo -n "$i: "
  dd if=/dev/urandom bs=8000000 count=100 2> /dev/null |
  ssh -c $i -o Compression=no ${remote} "(time -p cat) > /dev/null" > $tmp 2>&1
  
  if [[ $? == 0 ]]; then
      grep real $tmp | awk '{print 800 / $2" MB/s" }'
  else
